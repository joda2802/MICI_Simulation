...
#Geometrie erstellen
points = [gmsh.model.occ.add_point(x=float(data[i][1]), y=float(data[i][2]), z=0) for i in range(1,length) ]
#avg_point = gmsh.model.occ.add_point(x=np.mean([float(data[i][1]) for i in range(1,length)]), y=np.mean([float(data[i][2]) for i in range(1,length)]), z=0)
...
# Rectangle
gmsh.model.addPhysicalGroup(dim=gdim, tags=[area], tag=1) # Tag for area
gmsh.model.addPhysicalGroup(dim=gdim-1, tags=range(1,2), tag=2) # Tag for bottom
gmsh.model.addPhysicalGroup(dim=gdim-1, tags=range(2,4), tag=3) # Tag for cliff
gmsh.model.addPhysicalGroup(dim=gdim-1, tags=range(4,5), tag=4) # Tag for surface
gmsh.model.addPhysicalGroup(dim=gdim-1, tags=range(5,6), tag=5) # Tag for inflow

#Netz
h_size = 50 #Elemengröße
h_size_fine = 10 #verfeinerte Elementgröße
